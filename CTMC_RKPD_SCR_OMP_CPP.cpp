...
enum waveformType {Gaussian, cosSquare};
...
typedef struct data {
		...
		long double Period, RandNum_bmax;
                int iValue, iSize;
                long double dElectron_prefactor, dTarget_prefactor;
                long double Dispersion, fi_CEP, dAmplitude_y, dAmplitude_z, dPolarisation_phase ,dWavelength, dPulseWidth, /*dX_sigma, dX_twosigmasquare,*/ dT_sigma, dT_twosigmasquare, Wavevector, AngularFrequency, dTau, c, tBegin, tEnd, DispersionMax;
                waveformType eWaveform;
...
void crossProduct(long double* input1, long double* input2, long double* result){
...
void calc_eccentricity(DATA* parameters, long double* eccentricity, long double* xx, long double* y_t){
	long double l_momentum, l[3];
        crossProduct(&y_t[electron_x], &y_t[electron_vx], l);
...
void Lorentz_simple_calc(DATA* parameters, long double* t, long double* y_t, long double* dEplusvxB, long double* dEplusvxB_electron, long double* Electric, long double* fi_CEP){
long double cose, cose_electron, timesquare, timesquare_electron, dCarrier, dCarrier_electron, AngularFrequency = parameters->AngularFrequency+parameters->Dispersion*(*t), AngularFrequency_electron = parameters->AngularFrequency+parameters->Dispersion*((*t)-y_t[electron_x]/parameters->c); 

long double trigonometricFuncParam = AngularFrequency*(*t) + *fi_CEP, trigonometricFuncParam_electron = AngularFrequency_electron*((*t)-y_t[electron_x]/parameters->c) + *fi_CEP;

long double arg[] = {cos(trigonometricFuncParam + parameters->dPolarisation_phase), cos(trigonometricFuncParam)}, arg_electron[] = {cos(trigonometricFuncParam_electron + parameters->dPolarisation_phase), cos(trigonometricFuncParam_electron)};
long double dElectric[3], dB[3], dElectric_electron[3], dB_electron[3], vxB_electron[3];

	switch (parameters->eWaveform) {
		case Gaussian:
		timesquare = (*t)*(*t)/parameters->dT_twosigmasquare;
		timesquare_electron = pow((*t) - y_t[electron_x]/parameters->c, 2)/parameters->dT_twosigmasquare;
		dCarrier = sqrt(exp(-timesquare));
		dCarrier_electron = sqrt(exp(-timesquare_electron));

		dElectric[0] = 0;
		dElectric[1] = parameters->dAmplitude_y*dCarrier*arg[0];
		dElectric[2] = parameters->dAmplitude_z*dCarrier*arg[1];

		dElectric_electron[0] = 0;
		dElectric_electron[1] = parameters->dAmplitude_y*dCarrier_electron*arg_electron[0];
		dElectric_electron[2] = parameters->dAmplitude_z*dCarrier_electron*arg_electron[1];

        	break;
        	case cosSquare:
		if(((*t) < (-parameters->dTau/2)) || ((*t) > (parameters->dTau/2))) { dElectric[0] = 0; dElectric[1] = 0; dElectric[2] = 0; }
		else {
			cose = cos(M_PI * (*t) / parameters->dTau);
			dCarrier = pow(cose, 2);
	...
	if((((*t) - y_t[electron_x]/parameters->c) < (-parameters->dTau/2)) || (((*t) - y_t[electron_x]/parameters->c) > (parameters->dTau/2))) { dElectric_electron[0] = 0; dElectric_electron[1] = 0; dElectric_electron[2] = 0;}
	else {
                cose_electron = cos(M_PI * ((*t) - y_t[electron_x]/parameters->c) / parameters->dTau);
                dCarrier_electron = pow(cose_electron, 2);
...
//	crossProduct( &y_t[electron_vx], dB_electron, vxB_electron);
	...
	Electric[0] = dElectric[1];
	Electric[1] = dElectric[2];
}

void coefficients_simple(DATA* parameters, long double y_t[], long double* t_giv, long double k_giv[], long double k_calc[], long double dt, long double* fi_CEP){
	long double ycalc_t[parameters->iSize], F_A, A, FACTAA, squareA, cubicA, dEplusvxB[3], dEplusvxB_electron[3], Electric[2];
        ...
        Lorentz_simple_calc(parameters, t_giv, ycalc_t , dEplusvxB, dEplusvxB_electron, Electric, fi_CEP);
	...
	F_A=(1/parameters->RedukaltTomeg)*parameters->iz_electron/squareA*FACTAA;

        k_calc[electron_x]=ycalc_t[electron_vx]*dt;
        k_calc[electron_y]=ycalc_t[electron_vy]*dt;
        k_calc[electron_z]=ycalc_t[electron_vz]*dt;

        k_calc[electron_vx]=(F_A*ycalc_t[electron_x]+parameters->dElectron_prefactor*dEplusvxB[0]-parameters->dTarget_prefactor*dEplusvxB[0])*dt;
        k_calc[electron_vy]=(F_A*ycalc_t[electron_y]+parameters->dElectron_prefactor*dEplusvxB[1]-parameters->dTarget_prefactor*dEplusvxB[1])*dt;
        k_calc[electron_vz]=(F_A*ycalc_t[electron_z]+parameters->dElectron_prefactor*dEplusvxB[2]-parameters->dTarget_prefactor*dEplusvxB[2])*dt;
}

void collision_process(DATA* parameters, long double* y_t, int *validity, IonisationStatistics* IonisationStatist, int *k, long double* time_of_ionization, long double* time_of_ionization_prev, long double* AngularFrequency_of_ionization, long double* fi_CEP){
	void (*coefficientsPtr)(DATA* , long double*, long double*, long double*, long double*, long double, long double*);
        const long double dt_original = 1e-6;
        long double t = parameters->tBegin, E_All, t_in, k_in[parameters->iSize], k1[parameters->iSize], k2[parameters->iSize], k3[parameters->iSize], k4[parameters->iSize], k5[parameters->iSize], k6[parameters->iSize], k7[parameters->iSize], zk[parameters->iSize], fk[parameters->iSize]/*, s[iValue]*/, epsilon=1e-6;
	long double dt=dt_original, R_max, delta, R[parameters->iSize], E_kin, E_pot;
        int i, j, errorCounter = 0, errorMax = 1000 * parameters->dPulseWidth, ionizationCheck = 0;
	coefficientsPtr = &coefficients_simple;
	parameters->iValue = 6;

for(i=0; t < (parameters->tEnd); i++){

	for(j=0; j<parameters->iValue; j++) { k_in[j]=0; }
	(*coefficientsPtr)(parameters, y_t, &t, k_in, k1, dt, fi_CEP);

	t_in = t+dt/5;
	for(j=0; j<parameters->iValue; j++) { k_in[j]=k1[j]/5; }
	(*coefficientsPtr)(parameters, y_t, &t_in, k_in, k2, dt, fi_CEP);

	t_in = t+3*dt/10;
	for(j=0; j<parameters->iValue; j++) { k_in[j]=3*k1[j]/40+9*k2[j]/40; }
	(*coefficientsPtr)(parameters, y_t, &t_in, k_in, k3, dt, fi_CEP);

	t_in = t+4*dt/5;
	for(j=0; j<parameters->iValue; j++) { k_in[j]=44*k1[j]/45-56*k2[j]/15+32*k3[j]/9; }
	(*coefficientsPtr)(parameters, y_t, &t_in, k_in, k4, dt, fi_CEP);

	t_in = t+8*dt/9;
	for(j=0; j<parameters->iValue; j++) { k_in[j]=19372*k1[j]/6561-25360*k2[j]/2187+64448*k3[j]/6561-212*k4[j]/729; }
	(*coefficientsPtr)(parameters, y_t, &t_in, k_in, k5, dt, fi_CEP);

	t_in = t+dt;
	for(j=0; j<parameters->iValue; j++) { k_in[j]=9017*k1[j]/3168-355*k2[j]/33+46732*k3[j]/5247+49*k4[j]/176-5103*k5[j]/18656; }
	(*coefficientsPtr)(parameters, y_t, &t_in, k_in, k6, dt, fi_CEP);

        for(j=0; j<parameters->iValue; j++) { k_in[j]=35*k1[j]/384+500*k3[j]/1113+125*k4[j]/192-2187*k5[j]/6784+11*k6[j]/84; }
	(*coefficientsPtr)(parameters, y_t, &t_in, k_in, k7, dt, fi_CEP);
	...
	if( R_max <= epsilon ){
		...
		E_All = E_kin + E_pot;
		if((E_All >= 0) && (ionizationCheck == 0)) {*time_of_ionization = t; *AngularFrequency_of_ionization = parameters->AngularFrequency+ t * parameters->Dispersion; ionizationCheck = 1;}
		t+=dt;
		for(j=0; j<parameters->iValue; j++){
			y_t[j]=fk[j];
		}
	...
	if(errorCounter > errorMax ) {*validity=0; goto end_of_function; /*dt=1e-8; flag=1;*/}
...
void Statistics(DATA* parameters, IonisationStatistics* local_IonisationStatist ,long double* y_t, int validity, int Num, int* k, long double* time_of_ionization, long double* AngularFrequency_of_ionization, long double* fi_CEP){
	int my_rank;
	long double E_Te, E_Pe, E_kin_Te, E_pot_Te, E_kin_Pe, E_pot_Pe;
	...
	if(validity==1){
		...
		if(E_Te<0) { (local_IonisationStatist->excitation)++; (*k)++; }
			...
			(local_IonisationStatist->ionization)++;
			...
			electronData.push_back(*AngularFrequency_of_ionization);
                    	electronData.push_back(*fi_CEP); 
...
void distribution(int iCount, int iRemainder, int thread_count, DATA* parameters, IonisationStatistics* IonisationStatist){
	...
	int k, i, validity, iNumjob_per_Thread;
	long double local_rDistribution[parameters->meretrDistribution], local_energyDistribution[parameters->meretrDistribution], y, fi_CEP, eccentricity, xx, y_t[parameters->iSize], time_of_ionization, time_of_ionization_prev, AngularFrequency_of_ionization;
	...
	for(k=0; iNumjob_per_Thread > k; ){
		...
		calc_eccentricity(parameters, &eccentricity, &xx, y_t);
			if((y!=0) && (xx>=0) && (eccentricity < 0.999)) {
				fi_CEP = (long double) 2 * M_PI * rand()/RAND_MAX;
				collision_process(parameters, y_t, &validity, IonisationStatist, &k, &time_of_ionization, &time_of_ionization_prev, &AngularFrequency_of_ionization, &fi_CEP);
				Statistics(parameters, &local_IonisationStatist ,y_t, validity, iNumjob_per_Thread, &k, &time_of_ionization, &AngularFrequency_of_ionization, &fi_CEP);
			}
	}
#pragma omp critical
	{
		IonisationStatist->ionisedElectron.insert(std::end(IonisationStatist->ionisedElectron), std::begin(local_IonisationStatist.ionisedElectron), std::end(local_IonisationStatist.ionisedElectron));
...
void StatisticsKiir(DATA* parameters, IonisationStatistics* IonisationStatist){
        ...
        std::string waveform ,fileName = std::string("Statistics_") + parameters->additionalInfo + '_' + std::to_string(parameters->Dispersion) + std::string(".txt");
	...
	myfile << "#DispersionMax: " << parameters->DispersionMax << "\t Dispersion: "<< parameters->Dispersion << '\n';
	...
	myfile << "# ex\tey\tez\tvx\tvy\tvz\tpx\tpy\tpz\tEtotal\tEkin\tEpot\ttime_of_ionization\tAngular_frequency_of_ionization\tfi_CEP\n";
...
void ElectricField_save(DATA* parameters){
	int i;
	long double t = parameters->tBegin, fi_CEP = 0;
	double dt = 1e-3/parameters->AngularFrequency;
	long double dEplusvxB, dEplusvxB_electron, Electric[2];
	long double y_t[parameters->iSize];
	
	for(i = 0; i < parameters->iSize; i++) {y_t[i] = 0;}

	std::ofstream Electric_file;
	std::string fName = std::string ("ElectricField_") + std::to_string(parameters->Dispersion) + std::string(".txt");
	Electric_file.open(fName);
	Electric_file << "t\tEy\tEz\n";
		
	while( t < (parameters->tEnd) ){
		Lorentz_simple_calc(parameters, &t, y_t, &dEplusvxB, &dEplusvxB_electron, Electric, &fi_CEP);
		Electric_file << t << '\t' << Electric[0] << '\t' << Electric[1] << '\n';
		t += dt;
	}
...
class Electron{
	public:
		DATA parameters;
                Electron(int z_electron, long double m_electron, int z_target, long double m_target, int N_electron, long double* Amplitude, long double Wavelength, long double PulseWidth, long double fi_CEP, long double Polarisation_phase ,waveformType eWaveform) {
				...
				parameters.iN_electron = N_electron;
                                parameters.dAmplitude_y = Amplitude[0];
                                parameters.dAmplitude_z = Amplitude[1];
                                parameters.dWavelength = Wavelength;
                                parameters.dPulseWidth = PulseWidth;
                                parameters.fi_CEP = fi_CEP;

                                parameters.dPolarisation_phase = Polarisation_phase;
                                parameters.c = 137.04;
                                ...
                                parameters.eWaveform = eWaveform;

                                parameters.Wavevector = 2 * M_PI / (18.897 * parameters.dWavelength);
                                parameters.AngularFrequency = parameters.c * parameters.Wavevector;
				//printf("Angular frequency: %Lf\n", parameters.AngularFrequency);

                                switch (parameters.eWaveform) {
                                case Gaussian:
                                    parameters.dT_sigma = 41.341*parameters.dPulseWidth/sqrt(8 * log (2));
                                    parameters.dT_twosigmasquare = 2*pow(parameters.dT_sigma, 2);
				    parameters.tBegin = -6 * parameters.dT_sigma;
				    parameters.tEnd = 6 * parameters.dT_sigma;
				    parameters.DispersionMax = fabs(parameters.AngularFrequency/parameters.tBegin);
                                    break;
                                case cosSquare:
                                    parameters.dTau = 41.341*parameters.dPulseWidth*M_PI/(2*acos(pow(0.5,0.25)));
				    parameters.tBegin = -0.5 * parameters.dTau;
				    parameters.tEnd = 0.8 * parameters.dTau;
				    parameters.DispersionMax = fabs(parameters.AngularFrequency/parameters.tBegin);
                                    break;
                                default:
                                    break;
				...
				if(parameters.r == NULL) std::cout << "Not enough memory for 'r' variable." << std::endl;
				
				parameters.Fuggveny = new long double [parameters.meret];
				if(parameters.Fuggveny == NULL) std::cout << "Not enough memory for 'Fuggveny' variable." << std::endl;;
				
				parameters.OmegaR = new long double [parameters.meret];
				if(parameters.OmegaR == NULL) std::cout << "Not enough memory for 'OmegaR' variable." << std::endl;
				
				parameters.rDistribution = new long double [parameters.meretrDistribution];
				if(parameters.rDistribution == NULL) std::cout << "Not enough memory for 'rDistribution' variable." << std::endl;
				
				parameters.energyDistribution = new long double [parameters.meretrDistribution];
				if(parameters.energyDistribution == NULL) std::cout << "Not enough memory for 'energyDistribution' variable."<< std::endl;
				
				parameters.error_of_rDistribution = new long double [parameters.meretrDistribution];
				if(parameters.error_of_rDistribution == NULL) std::cout << "Not enough memory for 'error_of_rDistribution' variable." << std::endl;
				
				parameters.error_of_energyDistribution = new long double [parameters.meretrDistribution];
				if(parameters.error_of_energyDistribution == NULL) std::cout << "Not enough memory for 'error_of_energyDistribution' variable." << std::endl;
...
int main(int argc, char* argv[]){
	...
	t=clock();
        ...
        int Num = 1200, iDispersionCounter=1, iD, iCount, iRemainder;
        ...
        long double Amplitude[] = {0, 0.136093513313091} /*Amplitude in Hartree atomic units*/, Wavelength = 800 /*Wavelength in nanometers*/, PulseWidth = 7/*1.0994*/ /*Pulsewidth in femtoseconds*/, fi_CEP=0 /* carrier envelope phase in deg */, Dispersion=0, 	Polarisation_Phase=0 /*phase difference of the two component of the polarisation in deg. In case of 90 it is circularly (right) polarised. In case of 270 it is circularly (left) polarised, provided the two components of Amplitudes are the same.*/;

        class IonisationStatistics IonisationStatist;

		class Electron atom(z_electron, m_electron, z_target, m_target, N_electron, Amplitude, Wavelength, PulseWidth, M_PI*fi_CEP/180, M_PI*Polarisation_Phase/180 , Gaussian);

	for( iD = -iDispersionCounter; iD <= iDispersionCounter; iD++){
		IonisationStatist.initialize();
        	atom.parameters.Dispersion = (long double) iD/iDispersionCounter * atom.parameters.DispersionMax;
        	ElectricField_save(&atom.parameters);
